...
class UserModel
{
    ...
    function register($nama, $email, $no_hp, $password) //fungsi register
    {
        $insert = mysqli_query($this->con, "INSERT INTO user VALUES ('','$nama','$email','$no_hp','$password')"); //proses insert ke database
        return $insert; // mengembalikan nilai insert
    }

    function login($email, $password) //fungsi login
    {
        $logged = mysqli_query($this->con, "SELECT * from user WHERE email = '$email' AND password = '$password'"); //mengambil data ke database berdasarkan inputan
        if ($logged->num_rows >= 1) { //validasi apakah data tersedia
            $data_user = $logged->fetch_array(); //menyimpan hasil query ke dalam sebuah variabel
            $_SESSION['nama'] = $data_user['nama']; //save data ke dalam session
